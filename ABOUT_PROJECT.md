# 📖 About This Project

## The Origin Story: December 2020

### The Competition

In Fall 2020, Wake Forest University's Master of Science in Management (MSM) program partnered with **E. & J. Gallo Winery** for a semester-long market research case competition. Gallo, one of the world's largest wine producers, was watching Millennials and Gen Z consumers shift away from traditional wine and spirits toward a new category: **hard seltzers**.

The challenge was clear: *"How should Gallo respond to the spiked seltzer trend?"*

### Our Team's Approach

Our team (Group 25) chose to focus on **Barefoot Hard Seltzer's potential entry into the Chinese market**. At the time, hard seltzers were exploding in North America:
- White Claw dominated with ~60% market share
- The category was growing at 200-300% annually
- Brands like Truly, Bon & Viv, and Bud Light Seltzer were rushing in
- Total North American market: ~$4-5 billion

Meanwhile, China represented an untapped opportunity with a massive consumer base and growing interest in Western drinking culture.

### The Research

We designed and executed a comprehensive market research project:

**Primary Research:**
- **273 survey responses** from Chinese consumers aged 21-35
- Mix of current hard seltzer drinkers and potential consumers
- Questions covering taste preferences, packaging, pricing, purchasing channels
- Demographic and behavioral segmentation

**Secondary Research:**
- Industry reports on hard seltzer trends
- Competitor analysis (White Claw, Truly, local Chinese brands)
- Cultural factors affecting alcohol consumption in China
- E-commerce vs. retail channel dynamics

**Key Insights from 2020:**
- 72% preferred glass bottles (premium perception)
- Strong interest in fruit flavors, especially tropical
- E-commerce was critical for product discovery
- Wine-based positioning could differentiate in crowded market
- Target demographic: urban females, 25-35, middle-class

### Our Strategic Recommendations

We presented a **wine-based positioning strategy** for Barefoot Hard Seltzer:

1. **Unique Selling Proposition:** "Premium wine-based seltzer" (vs. malt-based competitors)
2. **Target Market:** Urban Chinese females, 25-35, health-conscious
3. **Distribution:** E-commerce first (Tmall, JD.com), followed by retail
4. **Pricing:** Premium positioning ($3-4 per can, 15% above competitors)
5. **Marketing:** "Light, elegant, sophisticated" - playing to wine heritage
6. **Projected Market Size:** $5 billion by 2025 in China

### The Victory

In December 2020, our team **won first place** in the Gallo Case Competition. 🏆

The judges praised our thorough consumer research, data-driven insights, and innovative wine-based positioning strategy. We believed we had found a winning formula: leverage Gallo's wine expertise to differentiate in a crowded market.

We were confident. We had the data. We had the strategy.

**We were also wrong about some critical assumptions.**

---

## Five Years Later: Why Revisit This Project?

### What Actually Happened (2020-2025)

As I prepared my data analyst portfolio in late 2025, I wondered: *"Were we right? Did our predictions hold up?"*

Here's what I discovered:

**North America Hard Seltzer Market (2025):**
- Market grew to **$12 billion** (we predicted ~$10-12B) ✅
- White Claw's share dropped to **50-52%** (we predicted ~60%) ✅
- **High Noon** (vodka-based) became the #1 spirit seltzer with 10-12% share
- **Barefoot Hard Seltzer** struggled with <2% market share ❌

**China Hard Seltzer Market (2025):**
- Market reached only **~$2 billion** (we predicted $5B) ❌
- Growth was slower than North America's trajectory
- Local brands (RIO, Horoyoi) dominated
- E-commerce represented 40%+ of sales ✅
- Wine-based seltzers showed no premium perception ❌

**The Critical Miss:**
Our core assumption—that wine-based positioning would be a differentiator—was fundamentally flawed. Consumer blind taste tests consistently showed that **people cannot distinguish alcohol bases** in flavored seltzers. The "wine advantage" we championed didn't matter to consumers at all.

Meanwhile, Gallo's **vodka-based High Noon** became a massive success by targeting Gen Z with aggressive pricing, multiple ABV options (4.5% and 8%), and "real fruit juice" positioning.

### Why This Matters for My Portfolio

Most data analyst portfolios showcase successful projects. **This one showcases both success and failure**, demonstrating:

1. **Intellectual Honesty:** Admitting when predictions were wrong
2. **Analytical Maturity:** Understanding that good data ≠ correct conclusions
3. **Growth Mindset:** Revisiting past work with fresh perspective
4. **Technical Evolution:** Rebuilding 2020 Excel analysis with Python/SQL/Tableau
5. **Critical Thinking:** Identifying flawed assumptions, not just calculation errors

In interviews, I can discuss not just technical skills, but how I:
- Validate assumptions before making recommendations
- Separate facts (data) from interpretations (strategy)
- Learn from mistakes systematically
- Apply statistical validation (MAPE methodology)

---

## My Role in the 2020 Project

**Team:** Wake Forest University MSM 2020, Group 25  
**Team Members:** Hongling Yu, Xin Yin, Niko Wang, Jinru Yao, Yitong Gao

**My Contributions:**
- Co-designed the 273-response consumer survey (Qualtrics platform)
- Conducted data analysis using Excel (pivot tables, descriptive statistics)
- Developed insights on consumer demographics and purchase behavior
- Contributed to market sizing and competitive analysis
- Participated in final presentation strategy and delivery

**Competition Result:** 🏆 First Place (out of 12 teams)

---

## The 2025 Portfolio Upgrade

### Why Now?

As I actively search for data analyst roles in Vancouver's competitive job market, I needed a portfolio that:
- Demonstrates real-world business thinking
- Shows technical skill progression over time
- Proves I can admit mistakes and learn from them
- Connects to local Vancouver market (White Claw HQ)
- Stands out from typical "analyzed Titanic dataset" portfolios

### What Changed?

**2020 Project:**
- Tools: Excel, PowerPoint, Qualtrics
- Analysis: Descriptive statistics, pivot tables, basic charts
- Deliverables: 30-slide PowerPoint deck
- Audience: Academic judges and client (Gallo)

**2025 Portfolio:**
- Tools: Python (pandas), SQL, Tableau, GitHub
- Analysis: Prediction validation (MAPE), cross-market comparison, statistical accuracy
- Deliverables: Interactive dashboard, GitHub repository, markdown documentation
- Audience: Hiring managers, technical recruiters, fellow analysts

### Time Investment

- **2020 Original Project:** ~60 hours over 8 weeks (team effort)
- **2025 Portfolio Rebuild:** ~35 hours over 7 days (solo work)

### New Analyses Added (2025)

1. **Prediction Validation Analysis**
   - Systematically compared 2020 predictions vs. 2025 reality
   - Calculated MAPE (Mean Absolute Percentage Error): 15%
   - Identified directional accuracy: 70% of predictions correct in direction
   - Analyzed qualitative misses (wine-based assumption)

2. **Cross-Market Dashboard (Tableau)**
   - Interactive 3-tab dashboard comparing North America vs China
   - Market size, growth, brand shares, consumer demographics
   - Channel distribution, pricing, competitive landscape
   - Filterable by year, region, metric

3. **High Noon Case Study** *(Optional addition)*
   - Why Gallo's vodka-based High Noon won while wine-based Barefoot struggled
   - Product comparison, target demographics, pricing strategy
   - Lessons learned about assumption testing

### Technical Skills Demonstrated

**Data Collection & Integration:**
- Synthesized data from multiple sources (Grand View Research, Statista, IWSR)
- Validated data consistency across different reports
- Documented all sources and methodologies

**Analysis:**
- Python pandas for data manipulation
- SQL queries for cross-market comparisons
- Statistical validation (MAPE, accuracy metrics)
- Qualitative vs. quantitative insights

**Visualization:**
- Tableau Public for interactive dashboards
- Chart selection based on data type and audience
- Professional color schemes and formatting

**Documentation:**
- GitHub repository structure
- Markdown for clear, accessible writing
- Data dictionaries and methodology docs
- Code commenting and version control

---

## Why This Project is Unique

### 1. Prediction Validation (Rare in Portfolios)

Most portfolios analyze historical data. This one:
- Tests predictions made 5 years ago
- Quantifies accuracy with statistical rigor (MAPE)
- Admits when wrong and explains why
- Demonstrates intellectual honesty over ego

### 2. Award-Winning Foundation

Starting with a competition-winning project:
- Proves baseline analytical capability
- Shows teamwork and presentation skills
- Demonstrates client-facing experience
- Adds credibility to portfolio

### 3. Cross-Market Expertise

Comparing North America vs. China shows:
- Global business thinking
- Cultural awareness in consumer behavior
- Ability to identify market timing opportunities
- Strategic insight beyond just numbers

### 4. Local Vancouver Relevance

Hard seltzer analysis is directly relevant to Vancouver because:
- **White Claw** (Mark Anthony Brands) is headquartered here
- White Claw is the market leader (50-52% share)
- Demonstrates knowledge of local business ecosystem
- Shows research into employers/industries in the region

### 5. Technical Evolution Story

Rebuilding the project shows:
- Clear skill progression (Excel → Python/SQL/Tableau)
- Self-directed learning ability
- Initiative to revisit and improve past work
- Commitment to professional development

---

## What I Learned

### About Data Analysis

**Good Data ≠ Good Decisions**
- We had 273 survey responses (solid sample size)
- We had industry reports (credible sources)
- We had logical reasoning (wine = premium)
- **But we missed a critical assumption test:** Do consumers actually care about alcohol base?

**Lesson:** Always validate core assumptions, not just collect data around them.

**Stated Preferences ≠ Actual Behavior**
- 72% said they preferred glass bottles
- Yet cans dominate actual sales (convenience)
- Survey responses can mislead if not validated

**Lesson:** Look for revealed preferences (behavior) over stated preferences (surveys).

**External Validation is Critical**
- We should have looked at global wine-based seltzer performance
- Other wine-based seltzers were struggling worldwide in 2019-2020
- This pattern was observable but we didn't check

**Lesson:** Always benchmark against global trends, not just local data.

### About Growth

This project taught me that:
- Making mistakes with good intentions is part of learning
- Intellectual humility beats ego in the long run
- Revisiting past work objectively is uncomfortable but valuable
- Admitting "I was wrong" is a strength, not weakness
- Growth mindset = treating failures as data, not shame

### About Portfolio Building

**What Works:**
- Real business context (not Kaggle toy datasets)
- Clear before/after story (2020 → 2025)
- Quantified results (15% MAPE, $12B market, 50% share)
- Professional documentation (GitHub, markdown)
- Visual storytelling (Tableau dashboard)

**What Doesn't:**
- Hiding mistakes or cherry-picking successes
- Generic "I analyzed X dataset" without unique angle
- Technical skills without business insight
- Claiming skills you don't actually have

---

## Impact & Next Steps

### For Job Applications

This portfolio positions me as:
- **Analytically mature:** Can handle being wrong professionally
- **Business-minded:** Connects data to strategy
- **Technically evolving:** Shows 5-year skill growth
- **Locally aware:** Understands Vancouver's business landscape
- **Self-motivated:** Rebuilt entire project independently

### For Interviews

I'm prepared to discuss:
- How I calculate and interpret MAPE
- What I'd do differently in 2020 (test assumptions)
- Cross-market consumer behavior insights
- Tableau dashboard design decisions
- How this experience shaped my analytical approach

### For Career Development

This project reinforced my interest in:
- Consumer behavior analytics
- Market research and strategy
- Cross-cultural business analysis
- Tableau visualization
- Survey design and statistical validation

---

## Connect With Me

**Niko Wang**  
📧 wzzclot@gmail.com  
🔗 [linkedin.com/in/zizhuo-wang-niko](https://www.linkedin.com/in/zizhuo-wang-niko)  
📍 Vancouver, BC, Canada

**Actively seeking Data Analyst roles in:**
- Market research & consumer insights
- Business intelligence & analytics
- Product analytics
- Cross-market analysis

**Let's discuss:**
- This project's insights and methodology
- Data analyst opportunities in Vancouver
- How to build impactful portfolios
- Market research and statistical validation

---

## Acknowledgments

**Original 2020 Team:**
- Hongling Yu, Xin Yin, Jinru Yao, Yitong Gao
- Wake Forest University MSM Program
- Professor [Course Instructor Name]

**2025 Portfolio:**
- Independent project (solo)
- Data from public industry research sources
- Built over 7 days in November 2025

**Special Thanks:**
- E. & J. Gallo Winery for the original case challenge
- Wake Forest University for the competition opportunity
- The 273 survey respondents from 2020
- Industry researchers (Grand View, Statista, IWSR) for publicly available data

---

**This is more than a portfolio project—it's a story about growth, humility, and the journey from student to professional analyst.**

*Last Updated: November 12, 2025*